...
import { User as UserTypeORMDataMapper } from "../models/UserTypeORMDataMapper";
...
router.get('/typeormddatamapper', async function(req: Request, res: Response, next) {
  const userRepository = AppDataSource.getRepository(UserTypeORMDataMapper)
  console.log(userRepository)

const user = new UserTypeORMDataMapper()
user.name = "Bob Dole"
...
const sean = await userRepository.findOneBy({
    name: "Sean Loughrey"
}) // find by name
